...
const metrics: {
  key: keyof WithTurnoverInv;
  label: string;
  fmt: (v: number, c: WithTurnoverInv) => string | number;
}[] = [
  ...
  {
    key: "headcount",
    label: "Perth headcount",
    fmt: (v) => Math.round(v as number).toLocaleString("en-US"),
  },
...
export function buildCompareRadar(idA: string | null, idB: string | null): CompareData | null {
  ...
  const pointsA: RadarPoint[] = metrics.map((m, i) => {
    ...
    return {
      cx: px.toFixed(1),
      cy: py.toFixed(1),
      leftPct: ((Number(lx) / 200) * 100).toFixed(2),
      topPct: ((Number(ly) / 208) * 100).toFixed(2),
      val: String(m.fmt(a[m.key] as number, a)),
    };
  });
  const pointsB: RadarPoint[] = metrics.map((m, i) => {
    ...
    return {
      ...
      leftPct: ((Number(lx) / 200) * 100).toFixed(2),
      topPct: ((Number(ly) / 208) * 100).toFixed(2),
      val: String(m.fmt(b[m.key] as number, b)),
    };
  });
