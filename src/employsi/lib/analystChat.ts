...
const LIMITS: Record<DataIntent, string> = {
  ...
  payBySkill:
    "What it can't tell you: what a skill is worth. It is what employers advertise for ads that MENTION the skill, and seniority, industry and employer size ride along with it — a skill that appears mostly in senior roles looks well paid because those roles are. An ad naming two skills counts in both, so neighbouring skills are not independent readings. And the list is skills with ENOUGH disclosed ads, not the best-paid skills full stop: anything under twelve disclosed ads is missing entirely, which in a thin scope can hide the actual top. Ask me the median for the whole scope to see what the premium is measured against, or the same question in another city — the disclosure skew is similar in both, so the ranking travels better than the levels.",
  duration:
