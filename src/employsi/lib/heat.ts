...
export function auHeatLegend(heat: HeatMetric): HeatLegend {
  ...
  if (heat === "salary")
    return { title: "Median salary · by city", lo: "$" + lo + "K", hi: "$" + hi + "K" };
  if (heat === "growth")
    return {
      title: "Employment growth · by city",
      lo: "+" + lo.toFixed(1) + "%",
      hi: "+" + hi.toFixed(1) + "%",
    };
...
export function globalHeatLegend(heat: HeatMetric): HeatLegend {
  ...
  const hi = Math.max(...vals);
  if (heat === "salary")
    return { title: "Median salary · by hub", lo: "$" + lo + "K", hi: "$" + hi + "K" };
  if (heat === "growth")
    return {
      title: "Employment growth · by hub",
      lo: "+" + lo.toFixed(1) + "%",
      hi: "+" + hi.toFixed(1) + "%",
    };
...
export function heatLegend(heat: HeatMetric): HeatLegend {
  ...
  if (heat === "salary") {
    ...
    return {
      title: "Median salary",
      lo: "$" + Math.round(Math.min(...v) / 1000) + "K",
      hi: "$" + Math.round(Math.max(...v) / 1000) + "K",
    };
  }
  if (heat === "growth") {
    ...
    return {
      ...
      lo: "+" + Math.min(...v).toFixed(1) + "%",
      hi: "+" + Math.max(...v).toFixed(1) + "%",
    };
  }
