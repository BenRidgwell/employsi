...
// The slice of Yahoo Finance's quoteSummary response this reads. Declaring the
// shape (rather than parsing into `any`) means a renamed field fails the build
// instead of quietly becoming 0 on every company card.
interface YahooRaw {
  raw?: number;
}
interface YahooQuoteSummaryResult {
  assetProfile?: { fullTimeEmployees?: number };
  financialData?: { totalRevenue?: YahooRaw; ebitda?: YahooRaw; financialCurrency?: string };
  price?: { marketCap?: YahooRaw; currency?: string };
}
interface YahooQuoteSummary {
  quoteSummary?: { result?: YahooQuoteSummaryResult[] };
}

async function queryYahoo(sym: string): Promise<YahooQuoteSummary | null> {
