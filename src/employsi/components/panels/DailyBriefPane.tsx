...
function moverQuery(risers: MarketSkillMover[], fallers: MarketSkillMover[]): string {
  ...
  const terms =
    '(jobs OR hiring OR "skills shortage" OR workforce OR demand OR layoffs OR redundancies OR automation)';
    ...
    <>
      ...
      <aside className={`briefpane ${open ? "open" : ""}`} aria-hidden={!open}>
        <div className="briefhead">
          ...
          <button className="briefclose" onClick={closeBrief} aria-label="Close">
            ✕
          </button>
        </div>
        ...
        <div className="briefscroll">
            ...
            <div className="briefmovers">
              ...
              <div className="briefmoverrows">
                {[
                  ...
                  ...fallers.slice(0, 2).map((m) => ({ m, dir: "down" as const })),
                ].map(({ m, dir }) => (
          ...
          {items.length === 0 ? (
            ...
            items.map((a, i) => {
              ...
              return (
                <a
                  className="briefcard briefcardlink"
                  key={a.url + i}
                  href={a.url}
                  target="_blank"
                  rel="noreferrer"
