...
const STOP = new Set(['of', 'and', 'the', 'for', '&', 'a', 'group', 'holdings', 'australia', 'pty', 'ltd']);
...
interface Sec { sector: string; group: string; skills: string[]; }
const RESOURCES = { sector: 'Resources', group: 'Energy & Natural Resources', skills: ['Mining Engineering', 'Heavy Diesel Maintenance', 'Plant & Equipment Operation', 'Safety & Compliance', 'Logistics & Supply Chain'] };
const CONSTRUCTION = { sector: 'Construction & Property', group: 'Infrastructure and Government', skills: ['Project Management', 'Civil Engineering', 'Construction Management', 'Trades & Services', 'Estimating & Contracts'] };
const CONSUMER = { sector: 'Consumer & Retail', group: 'Consumer and Retail', skills: ['Retail Operations', 'Supply Chain & Logistics', 'Sales & Business Development', 'Marketing & Comms', 'Customer Service'] };
const INDUSTRIAL = { sector: 'Industrial & Manufacturing', group: 'Industrial Manufacturing', skills: ['Manufacturing & Production', 'Mechanical Engineering', 'Warehousing & Logistics', 'Quality & Safety', 'Maintenance & Trades'] };
const TECH = { sector: 'Technology & Media', group: 'Technology, Media and Telecommunications', skills: ['Software Engineering', 'Data & Analytics', 'Product & Design', 'Sales & Business Development', 'Customer Success'] };
const HEALTH = { sector: 'Healthcare', group: 'Healthcare and Life Sciences', skills: ['Nursing', 'Allied Health', 'Aged & Community Care', 'Administration & Office Support', 'Health & Safety'] };
const FINANCE = { sector: 'Financial Services', group: 'Financial Services', skills: ['Finance & Accounting', 'Banking & Lending', 'Risk & Compliance', 'Data & Analytics', 'Sales & Business Development'] };
const TRANSPORT = { sector: 'Transport & Logistics', group: 'Industrial Manufacturing', skills: ['Driving & Transport', 'Warehousing & Logistics', 'Supply Chain & Logistics', 'Fleet & Maintenance', 'Operations Management'] };
const AGRI = { sector: 'Agribusiness', group: 'Consumer and Retail', skills: ['Agriculture & Farming', 'Supply Chain & Logistics', 'Food Trades', 'Warehousing & Logistics', 'Operations Management'] };

function classify(name: string): Sec {
  const n = name.toLowerCase();
  const has = (...k: string[]) => k.some((x) => n.includes(x));
  if (has('prospect', 'mining', 'resources', 'minerals', 'iron', 'gold', 'coal', 'petroleum', 'energy', 'oil', 'gas', 'fuel', 'lng')) return RESOURCES;
  if (has('construct', 'building', 'builder', 'property', 'developm', 'homes', 'civil', 'infrastructure', 'land', 'estate', 'concrete', 'steel')) return CONSTRUCTION;
  if (has('health', 'care', 'medical', 'hospital', 'pharma', 'dental', 'aged')) return HEALTH;
  if (has('capital', 'financial', 'invest', 'wealth', 'insurance', 'bank', 'finance')) return FINANCE;
  if (has('transport', 'logistics', 'freight', 'shipping', 'haul', 'linfox', 'cartage')) return TRANSPORT;
  if (has('tech', 'digital', 'software', 'media', 'telecom', 'data', 'gaming', 'games', 'online', 'entertainment')) return TECH;
  if (has('farm', 'agri', 'dairy', 'grain', 'cotton', 'poultry', 'meat', 'produce', 'wool', 'cattle', 'cbh')) return AGRI;
  if (has('manufactur', 'industr', 'packaging', 'plastics', 'motors', 'automotive', 'machinery', 'products', 'visy')) return INDUSTRIAL;
  if (has('food', 'beverage', 'wine', 'brands', 'retail', 'hospitality', 'hotel', 'liquor', 'supermarket', 'foods', 'fresh')) return CONSUMER;
  return CONSUMER;
}
...
export const TOP_PRIVATE_BY_CITY: Record<string, string[]> = BUILT.reduce<Record<string, string[]>>((acc, c) => {
