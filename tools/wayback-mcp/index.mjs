...
server.tool(
  ...
  "Find the closest Wayback Machine snapshot of a URL. Optionally target a moment in time with `timestamp` (YYYYMMDD or YYYYMMDDhhmmss). Returns the snapshot URL, its capture timestamp and HTTP status, or a note if none exists.",
  {
    ...
    timestamp: z
      .string()
      .optional()
      .describe("Preferred moment, YYYYMMDD[hhmmss]. Returns the nearest capture to it."),
  },
  async ({ url, timestamp }) => {
...
server.tool(
  ...
  'List Wayback Machine captures of a URL via the CDX API. Use matchType "prefix" or "host"/"domain" for wildcard-style crawls (e.g. all captures under careers.example.com/search). Returns rows of {timestamp, original, statuscode, mimetype, length}.',
  {
    url: z
      .string()
      .describe(
        "URL or URL pattern (host/path). With matchType prefix, captures everything beneath it.",
      ),
    matchType: z
      .enum(["exact", "prefix", "host", "domain"])
      .optional()
      .describe("exact (default), prefix, host, or domain."),
    ...
    limit: z
      ...
      .int()
      .optional()
      .describe("Max rows (default 50). Negative returns the most recent N."),
    filter: z
      .string()
      .optional()
      .describe('CDX filter, e.g. "statuscode:200" or "mimetype:text/html".'),
    collapse: z
      .string()
      .optional()
      .describe(
        'Collapse adjacent rows on a field, e.g. "digest" (dedupe identical captures) or "timestamp:8" (one per day).',
      ),
  },
...
server.tool(
  ...
  "Fetch the archived content of a specific Wayback snapshot. Give the original URL + its capture timestamp (from wayback_snapshots / wayback_available). Returns the raw page as it was captured.",
  {
    ...
    timestamp: z
      .string()
      .describe("The capture timestamp, YYYYMMDDhhmmss (from wayback_snapshots)."),
    raw: z
      .boolean()
      .optional()
      .describe(
        "True (default) fetches the original bytes with no Wayback rewriting (the id_ modifier).",
      ),
    maxChars: z
      .number()
      .int()
      .optional()
      .describe("Truncate the returned body to this many characters (default 20000)."),
  },
