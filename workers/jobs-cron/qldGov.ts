...
export function parseQldGovPage(html: string): {
  ...
  for (const body of blocks) {
    const titleM = body.match(
      /result-title"><strong>([\s\S]*?)<\/strong>\s*,?\s*([\s\S]*?)<\/span>/,
    );
    if (!titleM) continue;
    ...
    const hrefM =
      body.match(/href="([^"]+)"[^>]*>\s*<span class="result-title"/i) ||
    ...
    const locM =
      body.match(/class="locality">([\s\S]*?)<\/strong>/i) ||
      body.match(/<li>\s*<strong[^>]*>([\s\S]*?)<\/strong>/i);
