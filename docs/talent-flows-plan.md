...
| Timeline Mar 2006 – Jul 2026, scrubbable | events × hash | the delivery's measured period (2020-11 – 2025-10); the scrubber behaves as the skill card's (one handle starting at the latest month, a native range input over the track, fill from the start to the handle, the handle's month on the right, the event card following it); the card and map show **every move from the first month to the handle's**, rebuilt from `flow_months` by `viewForWindow` (same rules as the whole period; `check-flows` asserts the whole period reproduces it exactly). At the latest month that is the whole period | revised 2026-09-25 at the user's request, from "one window only"; then to the skill card's scrubber behaviour, which is why the window is start-to-handle (what the fill draws) rather than a trailing year. The rule-11 caveat still holds — the sample is today's employees, so earlier windows read lower by sampling alone (Rio Tinto inflow: 466 moves in the 5 years to 2008, 2,822 to 2025) — and the footnote states it whenever a window is shown |
