...
(function () {
  ...
  const HEAT = [
    ...
    { lon: 135.8, lat: -31.2, r: 0.1, rgb: "168,140,36" },
  ];
  ...
  function persist() {
    const out = {};
    PINS.forEach((p) => {
      out[p.code] = { lon: +p.lon.toFixed(3), lat: +p.lat.toFixed(3) };
    });
    try {
      window.localStorage.setItem(STORE, JSON.stringify(out));
    } catch (e) {}
  ...
  class SkillsGlobe extends HTMLElement {
    static get observedAttributes() {
      return ["spin-seconds", "spinseconds", "paused"];
    }
    ...
    connectedCallback() {
      ...
      this.readout.style.cssText =
        "position:absolute;left:50%;bottom:8px;transform:translateX(-50%);padding:4px 10px;border-radius:999px;background:rgba(28,28,30,.82);color:#fff;font-family:'JetBrains Mono',monospace;font-size:12px;letter-spacing:.04em;opacity:0;transition:opacity .2s ease;pointer-events:none";
      ...
      this.pinEls = PINS.map((p) => {
        ...
        const s = p.big ? 60 : 52;
        el.style.cssText =
          "position:absolute;left:0;top:0;display:flex;flex-direction:column;align-items:center;transition:opacity .35s ease;will-change:transform";
        el.innerHTML =
          '<div style="width:' +
          s +
          "px;height:" +
          s +
          "px;border-radius:999px;background:#fff;box-shadow:0 8px 18px -6px rgba(10,26,50,.55);display:flex;align-items:center;justify-content:center;" +
          "font-family:'JetBrains Mono',monospace;font-size:" +
          (p.big ? 19 : 17) +
          'px;font-weight:500;letter-spacing:.04em;color:#1c1c1e">' +
          p.code +
          "</div>" +
          ...
          (p.big ? 22 : 20) +
          'px;font-weight:500;color:#3a3a3c;white-space:nowrap;text-shadow:0 1px 3px rgba(255,255,255,.85)">' +
          p.name +
          "</div>" +
          (p.dx || p.dy
            ? '<div data-lead style="position:absolute;left:50%;bottom:' +
              (p.big ? 30 : 26) +
              'px;height:1.5px;background:rgba(255,255,255,.85);transform-origin:0 50%;box-shadow:0 0 3px rgba(10,26,50,.4)"></div>'
            : "");
    ...
    draw(lambda) {
      ...
      ctx.fill();
      ctx.lineWidth = 0.7;
      ctx.strokeStyle = "rgba(120,150,120,.45)";
      ctx.stroke();
      ...
      this.pinEls.forEach((el, i) => {
        ...
        el.style.transform =
          "translate(" +
          (pt[0] - (p.big ? 30 : 26) + (p.dx || 0)) +
          "px," +
          (pt[1] - (p.big ? 74 : 66) + (p.dy || 0)) +
          "px)";
        if (p.dx || p.dy) {
  ...
  if (!window.customElements.get("skills-globe"))
    window.customElements.define("skills-globe", SkillsGlobe);
})();
